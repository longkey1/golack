...
- `internal/slack/` — wrapper around `github.com/slack-go/slack`
  ...
  - `history.go` — `GetChannelHistory` (conversations.history, paged, inclusive unix bounds), `ListAllChannels` (conversations.list), `ResolveChannels` / `IsChannelID` (channel names vs `C…` IDs)
