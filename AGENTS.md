...
`gosla` is a Slack log collector CLI built with Cobra: it fetches Slack messages — a single message or thread by URL (`get`), a filtered search over a date range (`list`), or the complete history of given channels (`history`) — transforms them into a simplified thread-grouped JSON format (not the raw Slack API shape), and can merge and deduplicate previously collected JSON files (`merge`). `resolve` is an input-discovery helper: it converts between the IDs and names (users, channels, usergroups) that the other commands and the config take as input. Configuration (`token`, `author`, `mention`) is resolved with Viper from `$XDG_CONFIG_HOME/gosla/config.toml` (fallback `~/.config/gosla/config.toml`, override with `--config`), environment variables (`SLACK_API_TOKEN`, `SLACK_AUTHOR`, `SLACK_MENTION`), and CLI flags — in increasing order of precedence. Config file values may reference environment variables with `${VAR}`, expanded at load time.

## Build Commands

```sh
make build   # Build binary to ./bin/gosla
...
- `cmd/` — Cobra commands
  ...
  - `get.go` — `gosla get <url>`: fetch a single message, or the whole thread with `--thread` (or automatically when the URL carries `thread_ts=`)
  - `list.go` — `gosla list`: date-range search. `parseDateRange` validates the mutually exclusive `--day` / `--month` / `--from`+`--to` options (shared with `history`). Author/mention filters default from config; `--no-author` / `--no-mention` explicitly disable them. `processdays` runs one `collector.List` per day through a worker pool (`--parallel`)
  - `history.go` — `gosla history`: full channel history for a date range; `--channel` is required (names or IDs, resolved once up front via `ResolveChannels`), same date-range and worker-pool handling as `list`
  - `merge.go` — `gosla merge [dir]`: find JSON files (`--pattern`, `--recursive`), read them, merge via `collector.Merge`, print to stdout
  - `resolve.go` — `gosla resolve <query>...`: bidirectional ID<->name conversion for users/channels/usergroups. `classifyQuery` classifies each argument (ID by prefix, `#name`, `@name`, email, bare name) and intersects it with `--type`; lookups run through `slack.Directory`. Output is TSV lines (`type\tid\tname`) or a JSON array with `--json`; unresolved queries are stderr warnings, and the command fails only when nothing resolved
  - `config.go` — `gosla config get <key>` / `gosla config list`: show the effective configuration (valid keys: `token`, `author`, `mention`)
  ...
  - `version.go` — `gosla version`
- `internal/config/` — Viper-based loading (`Load`): TOML file (explicit path via `SetConfigFile`, else `defaultConfigPaths` = `$XDG_CONFIG_HOME/gosla` then `~/.config/gosla`), env bindings, `${VAR}` expansion via `os.ExpandEnv`. A missing file is only an error when `--config` was given explicitly. `Validate` requires a non-empty token
